...
export class PHQ9ScreeningsController {
  ...
  @Post()
  @CheckPolicies((ability) => ability.rulesFor(Action.Create, 'PHQ9Screening').some((r) => !r.inverted))
  create(@Body() dto: CreatePHQ9ScreeningDto, @CurrentUser() user: AuthenticatedUser) {
    return this.phq9ScreeningsService.createScreening(dto, user.id);
  }

  @Get()
  @CheckPolicies((ability) => ability.rulesFor(Action.Read, 'PHQ9Screening').some((r) => !r.inverted))
  ...
  @Patch(':id')
  @CheckPolicies((ability) => ability.rulesFor(Action.Update, 'PHQ9Screening').some((r) => !r.inverted))
