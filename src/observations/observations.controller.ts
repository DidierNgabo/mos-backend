...
export class ObservationsController {
  ...
  @Post()
  @CheckPolicies((ability) => ability.rulesFor(Action.Create, 'Observation').some((r) => !r.inverted))
  create(@Body() dto: CreateObservationDto, @CurrentUser() user: AuthenticatedUser) {
    return this.observationsService.createObservation(dto, user.id);
  }

  @Get()
  @CheckPolicies((ability) => ability.rulesFor(Action.Read, 'Observation').some((r) => !r.inverted))
  ...
  @Patch(':id')
  @CheckPolicies((ability) => ability.rulesFor(Action.Update, 'Observation').some((r) => !r.inverted))
