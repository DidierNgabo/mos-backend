...
@Injectable()
export class PHQ9ScreeningsMapper implements EntityMapper<
  PHQ9Screening,
...
> {
  ...
  fromCreateDto(dto: CreatePHQ9ScreeningDto): PHQ9Screening {
    ...
    s.id = randomUUID();
    s.initialOfParticipant = dto.initialOfParticipant;
    s.maritalStatus = dto.maritalStatus;
    s.educationLevel = dto.educationLevel;
    s.occupation = dto.occupation;
    s.division = dto.division;
    s.locationType = dto.locationType;
    s.religion = dto.religion;
  ...
  entityFromDto(dto: UpdatePHQ9ScreeningDto) {
    ...
    if (dto.initialOfParticipant !== undefined)
      patch.initialOfParticipant = dto.initialOfParticipant;
    if (dto.maritalStatus !== undefined)
      patch.maritalStatus = dto.maritalStatus;
    if (dto.educationLevel !== undefined)
      patch.educationLevel = dto.educationLevel;
    ...
    if (dto.q1LittleInterest !== undefined)
      patch.q1LittleInterest = dto.q1LittleInterest;
    if (dto.q2Depressed !== undefined) patch.q2Depressed = dto.q2Depressed;
    if (dto.q3SleepProblems !== undefined)
      patch.q3SleepProblems = dto.q3SleepProblems;
    if (dto.q4Fatigue !== undefined) patch.q4Fatigue = dto.q4Fatigue;
    if (dto.q5Appetite !== undefined) patch.q5Appetite = dto.q5Appetite;
    if (dto.q6Worthlessness !== undefined)
      patch.q6Worthlessness = dto.q6Worthlessness;
    if (dto.q7Concentration !== undefined)
      patch.q7Concentration = dto.q7Concentration;
    if (dto.q8Psychomotor !== undefined)
      patch.q8Psychomotor = dto.q8Psychomotor;
  ...
  filtersFromQueryDto(
    query: PHQ9ScreeningQueryDto,
  ): FilterQuery<PHQ9Screening>[] {
    const patientFilter: FilterQuery<PHQ9Screening> = query.patientId && {
      patient: { id: query.patientId },
    };
    const queueEntryFilter: FilterQuery<PHQ9Screening> = query.queueEntryId && {
      queueEntry: { id: query.queueEntryId },
    };
    const outreachFilter: FilterQuery<PHQ9Screening> = query.outreachId && {
      outreach: { id: query.outreachId },
    };
    const stationFilter: FilterQuery<PHQ9Screening> = query.stationId && {
      station: { id: query.stationId },
    };
    const recordedByFilter: FilterQuery<PHQ9Screening> = query.recordedById && {
      recordedBy: { id: query.recordedById },
    };
    const severityFilter: FilterQuery<PHQ9Screening> = query.severity && {
      severity: query.severity,
    };
    const createdAt: FilterQuery<PHQ9Screening> = query.createdAt && {
